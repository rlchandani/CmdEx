...
/// Custom NSPanel that can become key (for text field input).
...
final class PopoverManager {
    ...
    func show<V: View>(content: V, size: NSSize, from button: NSStatusBarButton) {
        ...
        // Focus the first text field after the panel is visible
        Task { @MainActor in
            try? await Task.sleep(for: .milliseconds(50))
            if let firstTextField = findFirstTextField(in: hosting) {
